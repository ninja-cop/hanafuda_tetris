...
class HanafudaTetris:
    ...
    def handle_input(self):
        """入力処理"""
        # 左右移動
        if pyxel.btnp(pyxel.KEY_LEFT) and self.falling_x > 0:
            if self.can_move(self.falling_x - 1, self.falling_y):
                self.falling_x -= 1
        if pyxel.btnp(pyxel.KEY_RIGHT) and self.falling_x < self.FIELD_WIDTH - 1:
            if self.can_move(self.falling_x + 1, self.falling_y):
                self.falling_x += 1
    ...
    def draw_game_over(self):
        ...
        pyxel.text(self.WIDTH // 2 - 40, self.HEIGHT // 2 + 20, "Press Enter to Restart", color)
