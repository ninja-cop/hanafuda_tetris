...
class HanafudaTetris:
    ...
    def handle_input(self):
        """入力処理"""
        # 左右移動
        if pyxel.btnp(pyxel.GAMEPAD1_BUTTON_DPAD_LEFT) and self.falling_x > 0:
            if self.can_move(self.falling_x - 1, self.falling_y):
                self.falling_x -= 1
        if pyxel.btnp(pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT) and self.falling_x < self.FIELD_WIDTH - 1:
            if self.can_move(self.falling_x + 1, self.falling_y):
                self.falling_x += 1
